...
class GameManager:
    ...
    async def notify_score(self, room_id, scoring_side, game_state):
        """
        ...
        if left_score >= SCORE_TO_WIN or right_score >= SCORE_TO_WIN:
            winner = "left" if left_score >= SCORE_TO_WIN else "right"
            await self.end_game(room_id, winner, game_state)
        else:
            # Broadcast the updated game state
            await self.broadcast_state(room_id, game_state)
